...
lol.y.lims <- c(-1, 703)
lol.y.brks <- c(0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700)
lol.y.labs <- c("", "50", "", "150", "", "250", "", "350", "", "450", "", "550", "", "650", "")
...
cc.pts <- c(121.8, 138.2, 130.3, 174.6)
cc.bench.pts <- c(55, 60.1, 44, 59.5)

cc.opp <- c("KC", "ES", "ZB", "KP")
cc.opp.pts <- c(140.9, 135.2, 137.4, 146.7)
cc.bench.opp.pts <- c(52.9, 59.9, 53.3, 93.4)
...
cs.pts <- c(132.4, 130.3, 136.3, 146)
cs.bench.pts <- c(62, 48.2, 32.9, 75.7)

cs.opp <- c("KP", "MR", "JR", "ES")
cs.opp.pts <- c(160.8, 162.9, 143.9, 137.6)
cs.bench.opp.pts <- c(56.5, 31.4, 33.6, 60)
...
es.pts <- c(166.4, 135.2, 134.6, 137.6)
es.bench.pts <- c(37.9, 59.9, 61.9, 60)

es.opp <- c("JR", "CC", "JW", "CS")
es.opp.pts <- c(91.9, 138.2, 118.8, 146)
es.bench.opp.pts <- c(41.8, 60.1, 90.4, 75.7)
...
jr.pts <- c(91.9, 151.4, 143.9, 166.2)
jr.bench.pts <- c(48.8, 62.5, 33.6, 42.1)

jr.opp <- c("ES", "JW", "CS", "MR")
jr.opp.pts <- c(166.4, 167.3, 136.3, 145.9)
jr.bench.opp.pts <- c(37.9, 71.2, 32.9, 42.3)
...
jw.pts <- c(139.9, 167.3, 118.8, 200.2)
jw.bench.pts <- c(70.2, 71.2, 90.4, 93.7)

jw.opp <- c("MR", "JR", "ES", "ZB")
jw.opp.pts <- c(185.3, 151.4, 134.6, 138.5)
jw.bench.opp.pts <- c(35.2, 62.5, 61.9, 30.9)
...
kc.pts <- c(140.9, 115.6, 129.2, 111.6)
kc.bench.pts <- c(52.9, 87.4, 52.7, 100.6)

kc.opp <- c("CC", "ZB", "KP", "SP")
kc.opp.pts <- c(121.8, 158, 165.7, 120.7)
kc.bench.opp.pts <- c(55, 110.4, 34.7, 24)
...
kp.pts <- c(160.8, 173.9, 165.7, 146.7)
kp.bench.pts <- c(56.5, 59.2, 34.7, 93.4)

kp.opp <- c("CS", "SP", "KC", "CC")
kp.opp.pts <- c(132.4, 151.4, 129.2, 174.6)
kp.bench.opp.pts <- c(62, 52.7, 52.7, 59.5)
...
mr.pts <- c(185.3, 162.9, 97.7, 145.9)
mr.bench.pts <- c(35.2, 31.4, 51.8, 42.3)

mr.opp <- c("JW", "ES", "SP", "JR")
mr.opp.pts <- c(139.9, 130.3, 121, 166.2)
mr.bench.opp.pts <- c(70.2, 48.2, 86, 42.1)
...
sp.pts <- c(163.6, 151.4, 121, 120.7)
sp.bench.pts <- c(68.5, 52.7, 86, 24)

sp.opp <- c("ZB", "KP", "MR", "")
sp.opp.pts <- c(112.7, 173.9, 97.7, 111.6)
sp.bench.opp.pts <- c(72.4, 59.2, 51.8, 100.6)
...
zb.pts <- c(112.7, 158, 137.4, 138.5)
zb.bench.pts <- c(72.4, 110.4, 53.3, 30.9)

zb.opp <- c("SP", "KC", "CC", "JW")
zb.opp.pts <- c(163.6, 115.6, 130.3, 200.2)
zb.bench.opp.pts <- c(68.5, 87.4, 44, 93.7)
...
master.line.p <- ggplot(data=master.team.df, aes(x=Week, y=Points, group=Team)) +
  ...
  scale_x_continuous(limits=c(1,4), breaks=c(1,2,3,4), labels=line.x.labs, expand=c(0,0.1)) +
...
master.lol.pts.p <- ggplot(data=master.tot.pts, aes(x=x, y=y)) +
  ...
  annotate("text", family="Courier", label="For", color="#33a02c", y=680, x=0.9, size=3) +
  annotate("text", family="Courier", label="Against", color="#d01c8b", y=680, x=1.6, size=3) +
...
save.image(file="zzWeek4_WS.Rdata")
